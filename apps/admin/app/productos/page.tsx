...
import { MOCK_PRODUCTOS, type Producto } from "@repo/lib";
...
export default function ProductosPage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // if (editingId) { await supabase.from('productos').update(form).eq('id', editingId); }
    // else { await supabase.from('productos').insert({ ...form }); }

    setTimeout(() => {
      if (editingId) {
        setProductos(prev => prev.map(p => p.id === editingId ? { ...p, ...form } : p));
      } else {
        const newId = `prod-${Date.now()}`;
        setProductos(prev => [...prev, { id: newId, ...form }]);
      }
      setSaving(false);
  ...
  const handleToggleActivo = (id: string) => {
    // TODO: await supabase.from('productos').update({ activo: !prod.activo }).eq('id', id);
    setProductos(prev => prev.map(p => p.id === id ? { ...p, activo: !p.activo } : p));
  };

  const handleDelete = (id: string) => {
    if (!confirm("¿Eliminar este producto?")) return;
    // TODO: await supabase.from('productos').delete().eq('id', id);
      ...
      <div className="space-y-6 animate-in fade-in duration-500 max-w-5xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[13px] text-zinc-500 mt-1">Catálogo visible en la web y app del cliente.</p>
          </div>
          <button
            onClick={openNew}
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white border border-zinc-200/60 rounded-xl p-4 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)]">
            <p className="text-[12px] text-zinc-500 mb-1">Total Productos</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {productos.map(prod => (
            <div
              key={prod.id}
              className={`bg-white border rounded-xl p-5 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] flex flex-col gap-3 transition-all ${
                prod.activo ? "border-zinc-200/60" : "border-zinc-100 opacity-50"
              }`}
            >
              {/* Header card */}
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-zinc-50 rounded-xl flex items-center justify-center text-2xl border border-zinc-100">
                    {prod.icon}
                  </div>
                  <div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 h-8 rounded-lg border border-zinc-200 text-[12px] font-bold text-zinc-700 bg-white hover:bg-zinc-50 transition-colors"
                >
                  ✏️ Editar
                </button>
                <button
                  onClick={() => handleToggleActivo(prod.id)}
                  className={`flex-1 h-8 rounded-lg border text-[12px] font-bold transition-colors ${
                    prod.activo
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-lg border border-red-100 text-red-500 bg-red-50 hover:bg-red-100 transition-colors flex items-center justify-center text-[13px]"
                >
                  🗑
                </button>
              </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 animate-in zoom-in-95 fade-in duration-200 text-zinc-900">
            ...
            <div className="flex items-center justify-between p-5 border-b border-zinc-100">
              <div>
                ...
                <p className="text-[12px] text-zinc-500 mt-0.5">
                  Se mostrará en la web y app del cliente
                </p>
              </div>
              <button
                onClick={() => setIsModalOpen(false)}
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
                  ...
                  {ICONOS.map(ic => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl text-xl border-2 transition-all ${
                        form.icon === ic ? "border-zinc-900 bg-zinc-100" : "border-zinc-200 hover:border-zinc-400"
                      }`}
                    >
                      {ic}
                    </button>
              ...
              <div>
                ...
                <input name="precio" type="number" required min={0} step={0.50} value={form.precio} onChange={handleChange} placeholder="45.00" className={inputClass} />
              </div>
